 ...
 **/
 func adminLog(){
     ...
     for{
        ...
        fmt.Println("请输入管理员用户名：");fmt.Scanln(&adminname)
        fmt.Println("请输入管理员密码：");fmt.Scanln(&password)
...
func admimManager(messages chan string){
    ...
    for{ 
        ...
        fmt.Scanln(&input)
        switch input {
            case "/to" : fmt.Println(userList())
                         fmt.Println("请输入聊天对象：") ; fmt.Scanln(&objUser)
                         if _,ok := conns[objUser];!ok{
                               fmt.Println("不存在此用户!")
                         }else{
                               fmt.Println("请输入消息：")  ; fmt.Scanln(&input)
                               notesInfo(messages,"To-Manager-"+objUser+"-"+input)
                        }
                         

           case "/all" : fmt.Println("请输入消息：")  ; fmt.Scanln(&input)
                         notesInfo(messages,"Manager say : " + input)

           case "/shield" :fmt.Println(userList())
                           fmt.Println("请输入屏蔽用户名：") ; fmt.Scanln(&objUser)
                           notesInfo(messages,objUser+"-/shield")
                           notesInfo(messages,"用户："+objUser+"已被管理员禁言！")

           case "/remove" :fmt.Println(userList())
                           fmt.Println("请输入踢出用户名：") ; fmt.Scanln(&objUser)
