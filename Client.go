...
func inputMessage(){  
    ...
    for {  
        ...
        switch input {
            ...
            // 向单个用户发送消息
            case "/to"   :  fmt.Println("请输入聊天对象：") ; fmt.Scanln(&objUser)
...
func clientLog(){
    ...
    LOOP: for {
        ...
        fmt.Println("请输入用户名：");fmt.Scanln(&username)
        if username == "" {
            goto LOOP
        }
        fmt.Println("请输入密码：");fmt.Scanln(&password)
...
func clientReg(){
    ...
    for {
        ...
        fmt.Println("请输入用户名：");fmt.Scanln(&username)
        fmt.Println("请输入密码：");fmt.Scanln(&pwd)
        fmt.Println("请确认密码：");fmt.Scanln(&pwdCheck)
